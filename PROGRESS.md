...
| Field | Value |
|-------|-------|
| **Latest completed slice** | v252 — expanded dungeon profile |
| **Active branch** | `main` |
| **CI gate** | v252 focused checks and `cd server && go test ./...` green on 2026-06-17; final `make ci` attempted and blocked by protocol bot scenario failures (`teleporter_lab`, `dungeon_monsters`, `boss_floor_gate`, `account_stash_storage`, `pack_aggro_and_dungeon_packs`, `ranger_piercing_and_pinning_shots`) |
| **Next slice** | Next feature slice via `$next` / `$autoloop`, using the v250 review follow-ups and deferred dungeon presentation/obstacle work |
