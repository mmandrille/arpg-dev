...
const STEP_TYPES_ACTION := [
	...
	"click_blacksmith_upgrade", "click_blacksmith_stage_item", "select_blacksmith_recipe", "select_blacksmith_tab", "fill_blacksmith_merge", "click_blacksmith_merge",
...
static func validate(step: Dictionary, stype: String, index: int) -> String:
	...
	if stype == "select_codex_page":
		return _require_string(step, index, stype, "page_id")
