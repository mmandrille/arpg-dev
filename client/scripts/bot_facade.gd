...
static func click_entity_id(main, target_id: String, buffered: bool = false) -> void:
	...
	if main.has_method("_send_action_intent"):
		if typ == "monster":
			CombatLocalAttackPresentation.present_local_start(_member(main, "_local_attack_presentation"), target_id, _member(main, "audio_controller"), _member(main, "player_anim"), "main_hand", CombatReach.local_player_attack_mode(_member(main, "inventory"), _member(main, "equipped")))
...
static func dispatch_action(main, intent_type: String, payload: Dictionary) -> void:
	...
	if intent_type == "move_to_intent":
		...
		if typeof(position) == TYPE_DICTIONARY and grace != null and grace.has_method("dispatch_or_queue_floor"):
			...
			return
	if main.has_method("_movement_intent_starts_motion") and MovementInputPresenterScript.intent_starts_motion(intent_type, payload):
