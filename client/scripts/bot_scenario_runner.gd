...
func _boss_health_bar_matches(step: Dictionary, state: Dictionary) -> bool:
	...
	if step.has("visible") and bool(bar.get("visible", false)) != bool(step.get("visible", true)):
		return false
	for key in ["boss_id", "boss_template_id", "title", "portrait_kind", "portrait_label", "reward_boss_template_id", "reward_title", "reward_status", "reward_hint"]:
	...
	if step.has("portrait_visible") and bool(bar.get("portrait_visible", false)) != bool(step.get("portrait_visible", true)):
		return false
	if step.has("reward_panel_visible") and bool(bar.get("reward_panel_visible", false)) != bool(step.get("reward_panel_visible", true)):
		return false
...
func _event_matches(step: Dictionary, event) -> bool:
	...
	for key in ["outcome", "source_entity_id", "target_entity_id", "shop_id", "offer_id", "item_instance_id", "skill_id", "damage_type", "service", "resource_id"]:
		if step.has(key) and str(ev.get(key, "")) != str(step.get(key, "")):
			return false
	if step.has("monster_def_id"):
		var want_monster := str(step.get("monster_def_id", ""))
		if str(ev.get("monster_def_id", "")) != want_monster and str(ev.get("source_monster_def_id", "")) != want_monster and str(ev.get("target_monster_def_id", "")) != want_monster:
			return false
...
func _blacksmith_panel_matches(step: Dictionary, state: Dictionary) -> bool:
	...
	if step.has("pity_threshold") and int(panel.get("pity_threshold", -1)) != int(step.get("pity_threshold", 0)):
		return false
	if step.has("pity_guaranteed") and bool(panel.get("pity_guaranteed", false)) != bool(step.get("pity_guaranteed", false)):
		return false
...
func _step_detail(step: Dictionary, stype: String) -> String:
	match stype:
		...
		"wait_bishop_panel", "assert_bishop_panel", "assert_bishop_panel_visible", "click_bishop_respec", "click_bishop_debug", "click_bishop_force_loot":
...
func _log_wait_progress(step: Dictionary, stype: String, state: Dictionary) -> void:
	BotWaitProgressLoggerScript.log_wait_progress(
		step, stype, state, _step_elapsed, _last_retry_at, str(scenario.get("id", "?")), _step_index
	)
