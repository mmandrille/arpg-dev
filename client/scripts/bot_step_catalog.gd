...
static func validate_step(step: Dictionary, index: int) -> String:
	...
	if stype in ["wait_market_panel", "assert_market_listing_rows"]:
		if not step.has("equals") and not step.has("at_least") and not step.has("price_gold") and not step.has("item_def_id") and not step.has("rolled") and not step.has("offer_equals") and not step.has("offer_at_least") and not step.has("offer_item_def_id") and not step.has("listing_item_def_id") and not step.has("offer_status"):
