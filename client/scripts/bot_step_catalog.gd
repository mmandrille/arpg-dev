...
const STEP_TYPES_WAIT := [
	...
	"click_entity_until_event", "wait_main_menu", "wait_character_panel",
		"wait_multiplayer_panel", "wait_settings_panel", "wait_codex_panel", "wait_pause_menu", "wait_character_progression",
...
const STEP_TYPES_ASSERT := [
	...
	"assert_character_panel_visible", "assert_settings_panel_visible", "assert_codex_panel",
...
const STEP_TYPES_ACTION := [
	...
	"enter_character_name", "select_character", "select_character_class", "select_codex_page", "select_window_size",
	...
	"click_stash_withdraw_gold", "click_bishop_respec", "click_bishop_debug", "click_bishop_force_loot", "set_stash_search", "select_stash_sort",
...
static func validate_step(step: Dictionary, index: int) -> String:
	...
	if stype in ["wait_mercenary_panel", "assert_mercenary_panel"]:
		if not step.has("visible") and not step.has("require_visible") and not step.has("price") and not step.has("gold") and not step.has("affordable") and not step.has("service_id") and not step.has("offer_id") and not step.has("monster_def_id") and not step.has("hired_entity_id") and not step.has("hired_count") and not step.has("candidate_count") and not step.has("selected_stance") and not step.has("status_contains") and not step.has("stats_card_contains") and not step.has("companion_bar_count") and not step.has("companion_icon_kind"):
