...
func ground_material_for_level(level: int) -> StandardMaterial3D:
	...
	var uv_scale := SurfaceMaterialLoaderScript.uv_scale(style_id, Vector2(28.0, 18.0))
	var mat := StandardMaterial3D.new()
	mat.albedo_texture = make_ground_texture(texture_id, palette)
	mat.albedo_color = Color.WHITE
	mat.roughness = SurfaceMaterialLoaderScript.scalar(style_id, "roughness", 0.88 if level != 0 else 0.92)
...
func ground_texel(texture_id: String, x: int, y: int, palette: Dictionary = {}) -> Color:
	...
	if texture_id == ClientConstantsScript.GROUND_TEXTURE_TOWN:
		var style := SurfaceMaterialLoaderScript.STYLE_TOWN_GROUND
		var base := SurfaceMaterialLoaderScript.color(style, "low", Color("#2f6136")).lerp(SurfaceMaterialLoaderScript.color(style, "high", Color("#79aa58")), float(n) / 16.0)
		var dirt_patch := int((x * 9 + y * 5 + ((x / 8) * 17) + ((y / 8) * 29)) % 41)
		if dirt_patch < 8:
			base = base.lerp(SurfaceMaterialLoaderScript.color(style, "soil", Color("#8f7447")), 0.50)
		elif dirt_patch < 13:
			base = base.lerp(SurfaceMaterialLoaderScript.color(style, "soil_dark", Color("#6f5f39")), 0.30)
		if ((x * 5 + y * 3) % 23) == 0:
			base = base.lerp(SurfaceMaterialLoaderScript.color(style, "accent", Color("#b7b56c")), 0.34)
		if ((x * 11 + y * 7) % 31) == 0:
			base = base.lerp(SurfaceMaterialLoaderScript.color(style, "dark", Color("#274c2b")), 0.36)
		if ((x / 8 + y / 8) % 5) == 0 and ((x * 13 + y * 17) % 19) < 3:
			base = base.lerp(SurfaceMaterialLoaderScript.color(style, "low", Color("#456f35")).lightened(0.12), 0.20)
		return base
	var style := SurfaceMaterialLoaderScript.STYLE_DUNGEON_FLOOR
	var low := _palette_color(palette, "ground_low", SurfaceMaterialLoaderScript.color(style, "low", Color("#3c3f43")))
	var high := _palette_color(palette, "ground_high", SurfaceMaterialLoaderScript.color(style, "high", Color("#73706b")))
	var crack := _palette_color(palette, "ground_crack", SurfaceMaterialLoaderScript.color(style, "dark", Color("#25282c")))
	var highlight := _palette_color(palette, "ground_highlight", SurfaceMaterialLoaderScript.color(style, "accent", Color("#a09a8e")))
...
func wall_material_for_level(level: int, source: String, size: Dictionary, wall_height: float = 1.0) -> StandardMaterial3D:
	var palette := biome_palette_for_level(level)
	var style_id := SurfaceMaterialLoaderScript.STYLE_DUNGEON_WALL
	var uv_scale := SurfaceMaterialLoaderScript.uv_scale(style_id, Vector2(2.0, 2.0))
	var mat := StandardMaterial3D.new()
	...
	mat.normal_scale = SurfaceMaterialLoaderScript.scalar(style_id, "normal_scale", 0.24)
	mat.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
	mat.roughness = SurfaceMaterialLoaderScript.scalar(style_id, "roughness", 0.94)
	mat.uv1_scale = Vector3(maxf(1.0, float(size.get("x", 1.0)) / uv_scale.x), maxf(1.0, wall_height / uv_scale.y), 1.0)
	var base := _palette_color(palette, "wall_base", SurfaceMaterialLoaderScript.color(style_id, "low", Color("#34363a")))
	var highlight := _palette_color(palette, "wall_highlight", SurfaceMaterialLoaderScript.color(style_id, "high", Color("#6b6255")))
	var mortar := _palette_color(palette, "wall_mortar", SurfaceMaterialLoaderScript.color(style_id, "dark", Color("#17191c")))
	match source:
		"generated":
...
func obstacle_material_for_level(level: int, kind: String, size: Dictionary) -> StandardMaterial3D:
	var palette := biome_palette_for_level(level)
	var style_id := SurfaceMaterialLoaderScript.STYLE_DUNGEON_COLUMN if kind == "column" else SurfaceMaterialLoaderScript.STYLE_DUNGEON_WALL
	var uv_scale := SurfaceMaterialLoaderScript.uv_scale(style_id, Vector2(2.0, 2.0))
	var mat := StandardMaterial3D.new()
	mat.albedo_texture = make_wall_texture(ClientConstantsScript.WALL_TEXTURE_CAVE, palette)
	mat.normal_enabled = true
	mat.normal_texture = make_wall_normal_texture(ClientConstantsScript.WALL_TEXTURE_CAVE, palette)
	mat.normal_scale = SurfaceMaterialLoaderScript.scalar(style_id, "normal_scale", 0.20)
	mat.texture_filter = BaseMaterial3D.TEXTURE_FILTER_NEAREST
	mat.roughness = SurfaceMaterialLoaderScript.scalar(style_id, "roughness", 0.98)
	mat.uv1_scale = Vector3(maxf(1.0, float(size.get("x", 1.0)) / uv_scale.x), maxf(1.0, float(size.get("y", 1.0)) / uv_scale.y), 1.0)
	var base := _palette_color(palette, "wall_base", SurfaceMaterialLoaderScript.color(style_id, "low", Color("#34363a")))
	var highlight := _palette_color(palette, "wall_highlight", SurfaceMaterialLoaderScript.color(style_id, "high", Color("#6b6255")))
	var mortar := _palette_color(palette, "wall_mortar", SurfaceMaterialLoaderScript.color(style_id, "dark", Color("#17191c")))
	match kind:
...
func water_texel(x: int, y: int, palette: Dictionary = {}) -> Color:
	var style := SurfaceMaterialLoaderScript.STYLE_WATER
	var low := _palette_color(palette, "water_low", SurfaceMaterialLoaderScript.color(style, "low", Color("#1f5f7a")))
	var high := _palette_color(palette, "water_high", SurfaceMaterialLoaderScript.color(style, "high", Color("#5fa8bb")))
	var ripple := int((x * 17 + y * 31 + ((x / 8) * 13) + ((y / 8) * 7)) % 19)
	var water := low.lerp(high, float(ripple) / 18.0)
	if abs((x % 16) - (y % 16)) <= 1:
		water = water.lerp(_palette_color(palette, "water_foam", SurfaceMaterialLoaderScript.color(style, "accent", Color("#a9d8df"))), 0.24)
	if ((x * 11 + y * 5) % 37) == 0:
		water = water.lerp(_palette_color(palette, "water_foam", SurfaceMaterialLoaderScript.color(style, "accent", Color("#c2eef1"))), 0.34)
	return water
...
func wall_texel(_texture_id: String, x: int, y: int, palette: Dictionary = {}) -> Color:
	...
	var style := SurfaceMaterialLoaderScript.STYLE_DUNGEON_WALL
	var base := _palette_color(palette, "wall_base", SurfaceMaterialLoaderScript.color(style, "low", Color("#34363a")))
	var highlight := _palette_color(palette, "wall_highlight", SurfaceMaterialLoaderScript.color(style, "high", Color("#6b6255")))
	var mortar := _palette_color(palette, "wall_mortar", SurfaceMaterialLoaderScript.color(style, "dark", Color("#17191c")))
