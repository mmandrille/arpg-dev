...
func _stat_lines_for_tooltip(stats: Dictionary, signed: bool) -> Array:
	...
	for key in ["str", "dex", "vit", "magic", "all_skills", "armor", "block_percent", "attack_speed_percent", "hit_chance", "crit_chance", "evade_chance", "max_hp", "max_mana", "health_regen_per_10_seconds", "mana_regen_per_10_seconds", "skill_damage_percent", "skill_cooldown_reduction_percent", "skill_mana_cost_reduction", "hotbar_slots", "inventory_rows"]:
		...
		if value == 0:
			continue
		lines.append("%s: %s" % [_display_stat(key), _format_stat_value(value, key == "block_percent" or key == "attack_speed_percent" or key == "hit_chance" or key == "crit_chance" or key == "evade_chance" or key == "skill_damage_percent" or key == "skill_cooldown_reduction_percent")])
	return lines
