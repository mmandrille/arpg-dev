...
func _set_loot_label_visible(loot_id: String, shown: bool, highlighted: bool = false) -> void:
	...
	if label != null:
		...
		label.modulate = _loot_filter.display_color(_loot_label_color(rec), highlighted, category, str(rec.get("rarity", "common")))
...
func _apply_upgrade_resource_response(result: Dictionary) -> void:
	...
	if resource_id == "":
		return
	if result.has("resource_inventory_count"):
		return
...
func get_bot_state() -> Dictionary:
	...
	var out := {
		...
		"damage_numbers": BotPresentationDebugScript.active_damage_numbers(damage_numbers_layer),
		"recent_damage_numbers": _bot_damage_numbers(),
...
func bot_select_blacksmith_tab(tab_name: String = "Merge") -> void:
	BotFacade.select_blacksmith_tab(self, tab_name)
func bot_fill_blacksmith_merge(count: int = 3) -> void:
	BotFacade.fill_blacksmith_merge(self, count)
func bot_click_blacksmith_merge() -> void:
