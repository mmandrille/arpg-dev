...
static func start(animation_player: AnimationPlayer, clip_name: String, attack_mode: String) -> bool:
	...
	_kill_existing(root)
	root.position = base + Vector3(0.0, 0.0, CombatFeelConfigScript.melee_lunge_distance())
	root.set_meta(META_BASE_POSITION, base)
	root.set_meta(META_COUNT, int(root.get_meta(META_COUNT, 0)) + 1)
	if root.is_inside_tree():
		var tween := root.create_tween()
		tween.tween_property(root, "position", base, CombatFeelConfigScript.melee_lunge_recovery_seconds()).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
...
static func get_debug_state(animation_player: AnimationPlayer) -> Dictionary:
	...
	return {
		"active": offset.length() > CombatFeelConfigScript.melee_lunge_settle_epsilon(),
