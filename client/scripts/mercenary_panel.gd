...
const SUPPORTED_STANCES := ["assist", "defend", "passive"]
...
func apply_stance_changed(ev: Dictionary) -> void:
	selected_stance = _normalize_stance(str(ev.get("stance", selected_stance)))
	if _status_label != null:
		_status_label.text = "Companions set to %s" % selected_stance.capitalize()
		_status_label.add_theme_color_override("font_color", Color("#9ee6a8"))
	_render()
...
func set_companions(next_companions: Array) -> void:
	...
	if not _companions.is_empty():
		selected_stance = _normalize_stance(str((_companions[0] as Dictionary).get("companion_stance", selected_stance)))
...
func show_roster(next_companions: Array) -> void:
	visible = true
	if _status_label != null and _status_label.text == "":
		_status_label.text = "Companion controls ready"
		_status_label.add_theme_color_override("font_color", Color("#9ee6a8"))
	set_companions(next_companions)
...
func _build() -> void:
	...
	_panel.configure("Companions", Vector2(PANEL_SIZE.x - 28, PANEL_SIZE.y - 52))
	...
	_stance_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_stance_label.add_theme_font_size_override("font_size", BODY_FONT_SIZE)
	_stance_label.add_theme_color_override("font_color", Color("#d9c8b5"))
	root.add_child(_stance_label)
	...
	for stance in SUPPORTED_STANCES:
		...
		button.text = stance.capitalize()
		button.custom_minimum_size = Vector2(98, 32)
		button.pressed.connect(func() -> void: _request_stance(stance))
		_stance_buttons[stance] = button
		stance_row.add_child(button)
...
func _render() -> void:
	...
	for stance in _stance_buttons.keys():
		var button: Button = _stance_buttons[stance]
		button.disabled = str(stance) == selected_stance
	if _roster_label != null:
		if _companions.is_empty():
			_roster_label.text = "Companion roster: none"
		else:
...
func _request_stance(stance: String) -> void:
	stance_requested.emit(_normalize_stance(stance))
