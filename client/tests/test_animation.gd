...
func _test_model_reaction_death_terminal() -> void:
	...
	_assert(root.find_child("DeathFlourish", false, false) != null, "death reaction adds flourish")
	var color := _reaction_mesh_color(root)
...
func _test_model_reaction_terminal_reset_restores_model() -> void:
	...
	_assert(root.find_child("DeathFlourish", false, false) == null, "reaction terminal reset clears flourish")
...
func _quat_delta(a: Quaternion, b: Quaternion) -> float:
	return absf(a.x - b.x) + absf(a.y - b.y) + absf(a.z - b.z) + absf(a.w - b.w)
