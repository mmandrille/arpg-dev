...
func _initialize() -> void:
	...
	_test_controller_eye_view_debug_state()
...
func _test_controller_eye_view_debug_state() -> void:
	CameraPresentationsLoaderScript.reset_for_tests()
	var ctrl := PlayerCameraControllerScript.new()
	var root := Node3D.new()
	get_root().add_child(root)
	...
	var cam := ctrl.get_gameplay_camera()
	var debug := ctrl.get_debug_state()
	_assert_eq("eye-view mode uses chest_view setting", str(debug.get("mode", "")), ClientSettingsScript.CAMERA_MODE_CHEST_VIEW)
	_assert_eq("eye-view projection is perspective", str(debug.get("projection", "")), "perspective")
	_assert_true("eye-view has positive configured eye height", float(debug.get("eye_height", 0.0)) > 0.0)
	_assert_true("eye-view camera is above hero root", cam.global_position.y > visual.global_position.y + 1.0)
	_assert_true("eye-view near_clip is configured and positive", float(debug.get("near_clip", 0.0)) > 0.0)
	_assert_true("eye-view camera stays scene-root mounted", cam.get_parent() == root)
	_assert_true("eye-view camera debug exposes eye light", float(debug.get("eye_light_energy", 0.0)) > 0.0)
	_assert_true("eye-view camera debug exposes light range", float(debug.get("eye_light_range", 0.0)) >= 4.0)
	root.queue_free()
	CameraPresentationsLoaderScript.reset_for_tests()
