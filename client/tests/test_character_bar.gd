...
func _test_resource_wallet_debug_state() -> void:
	...
	_assert_eq("wallet text", str(state.get("wallet_text", "")), "Upgrade 2")
	_assert_true("wallet tooltip name", str(state.get("wallet_tooltip", "")).contains("Upgrade Badge x2"))
...
func _test_material_wallet_window_state() -> void:
	...
	bar.set_resource_wallet({"upgrade_shard": 2})
	bar.open_wallet_window()
	state = bar.get_debug_state()
	...
	_assert_true("wallet window row text", str(window.get("text", "")).contains("Upgrade Badge x2"))
	_assert_true("wallet window storage text", str(window.get("text", "")).contains("Stored account-wide"))
	bar.set_resource_wallet({"upgrade_shard": 4})
	state = bar.get_debug_state()
	window = state.get("wallet_window", {})
	_assert_true("wallet window refreshed", str(window.get("text", "")).contains("Upgrade Badge x4"))
	bar.set_resource_wallet({"upgrade_shard": 0})
	state = bar.get_debug_state()
	window = state.get("wallet_window", {})
