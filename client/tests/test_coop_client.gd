...
func _test_character_panel_modes_for_v45() -> void:
	...
	var barbarian_features: Array = expanded.get("class_feature_lines", [])
	_assert_true("barbarian features include rage", " ".join(barbarian_features).find("Rage") >= 0)
	...
	var sorc_features := " ".join(sorc_state.get("class_feature_lines", []))
	_assert_true("sorcerer features include magic bolt", sorc_features.find("Magic Bolt") >= 0)
	_assert_true("sorcerer features include teleport", sorc_features.find("Teleport") >= 0)
...
func _test_skill_cast_payload_uses_direction_without_nearest_fallback() -> void:
	...
	var over_monster := main._skill_cast_payload("magic_bolt", "2001", Vector2(0.0, -3.0), false)
	_assert_eq("direction skill ignores monster target_id", over_monster.has("target_id"), false)
	var hotbar := main._skill_cast_payload("magic_bolt", "", Vector2(1.0, 0.0), false)
	_assert_eq("hotbar ground aim has no target", hotbar.has("target_id"), false)
	_assert_eq("hotbar ground aim uses direction", hotbar.has("direction"), true)
	main.player_anchor.queue_free()
	main.entities_root.queue_free()
	main.walls_root.queue_free()
	main.free()
