...
func _run() -> void:
	...
	bar.set_skill_id("heal")
	bar.set_skill_cooldowns([{"skill_id": "magic_bolt", "remaining_ticks": 30, "total_ticks": 40}])
	state = bar.get_debug_state()
	_assert_eq("other selected skill has no cooldown", int(state.get("remaining_ticks", -1)), 0)
	_assert_false("other selected skill cooldown hidden", bool(state.get("cooldown_visible", true)))
	bar.set_skill_id("magic_bolt")
	bar.set_skill_cooldowns([{"skill_id": "magic_bolt", "remaining_ticks": 30, "total_ticks": 40}])
	state = bar.get_debug_state()
	_assert_eq("returning to cooling skill restores remaining", int(state.get("remaining_ticks", -1)), 30)
	_assert_eq("returning to cooling skill restores total", int(state.get("total_ticks", -1)), 40)
	_assert_true("returning to cooling skill shows visual", bool(state.get("cooldown_visible", false)))
	_assert_false("returning to cooling skill stays disabled", bool(state.get("enabled", true)))
...
func _skill_rows(magic_rank: int, rage_rank: int = 0, heal_rank: int = 0, cleave_rank: int = 0, ice_shard_rank: int = 0, holy_shield_rank: int = 0, lightning_rank: int = 0) -> Array:
	return [
		...
		{"skill_id": "lightning", "rank": lightning_rank, "max_rank": 5, "can_spend": false},
