...
func _run() -> void:
	...
	_assert_eq("class-filtered skill count", (state.get("skill_ids", []) as Array).size(), 5)
	...
	_assert_eq("sorcerer fourth skill visible", str((state.get("skill_ids", []) as Array)[3]), "revive")
	_assert_eq("sorcerer fifth skill visible", str((state.get("skill_ids", []) as Array)[4]), "arcane_barrage")
