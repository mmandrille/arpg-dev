...
func _run() -> void:
	...
	_assert_eq("class-filtered skill count", (state.get("skill_ids", []) as Array).size(), 6)
	...
	_assert_eq("sorcerer third skill visible", str((state.get("skill_ids", []) as Array)[2]), "ice_shard")
	_assert_eq("sorcerer fourth skill visible", str((state.get("skill_ids", []) as Array)[3]), "ligthing")
	_assert_eq("sorcerer fifth skill visible", str((state.get("skill_ids", []) as Array)[4]), "revive")
	_assert_eq("sorcerer sixth skill visible", str((state.get("skill_ids", []) as Array)[5]), "arcane_barrage")
	...
	_assert_eq("paladin has four compact visible skills", paladin_skill_ids.size(), 4)
	...
	_assert_eq("paladin fourth local skill", str(paladin_skill_ids[3]), "sanctuary")
	...
	_assert_eq("rogue has two starter skills plus higher row skills", rogue_skill_ids.size(), 4)
	...
	_assert_eq("rogue passive skill visible", str(rogue_skill_ids[3]), "executioner")
