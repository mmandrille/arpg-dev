...
func _run() -> void:
	...
	var expected_sorcerer_ids := _visible_skill_ids_for_class("sorcerer")
	var actual_sorcerer_ids: Array = state.get("skill_ids", [])
	_assert_eq("class-filtered skill count", actual_sorcerer_ids.size(), expected_sorcerer_ids.size())
	_assert_eq("sorcerer visible skill order", actual_sorcerer_ids, expected_sorcerer_ids)
	_assert_eq("sorcerer skill visible", str(actual_sorcerer_ids[0]), "magic_bolt")
	...
	_assert_false("rank 0 tooltip omits next-rank damage", str(state.get("tooltip_body", "")).contains("next rank:"))
	_assert_false("tooltip omits next-rank section header", str(state.get("tooltip_body", "")).contains("Next rank:"))
	_assert_false("rank 0 tooltip omits next-rank rich damage", str(state.get("tooltip_rich_text", "")).contains("next rank:"))
	...
	_assert_true("passive tooltip labels description section", arcane_focus_tooltip.contains("\n\nDescription:\nMax mana: +8%"))
	...
	var expected_paladin_ids := _visible_skill_ids_for_class("paladin")
	_assert_eq("paladin visible skill count", paladin_skill_ids.size(), expected_paladin_ids.size())
	...
	_assert_true("heal tooltip includes inline next-rank percent", heal_tooltip.contains("next rank: +%d%% heal" % (heal_rank2_percent - 25)))
	...
	var expected_rogue_ids := _visible_skill_ids_for_class("rogue")
	_assert_eq("rogue visible skill count", rogue_skill_ids.size(), expected_rogue_ids.size())
	_assert_eq("rogue visible skill order", rogue_skill_ids, expected_rogue_ids)
	_assert_eq("rogue first starter skill", str(rogue_skill_ids[0]), "poison_stab")
	_assert_true("poison stab tooltip labels description section", SkillsPanelScript.tooltip_plain_body("poison_stab", 1, panel.skill_progression, panel.character_progression).contains("\n\nDescription:\nMelee"))
	...
	var magic_min_delta := magic_rank2_min - 300
	var magic_max_delta := magic_rank2_max - 225
	_assert_true("rank 1 tooltip includes next-rank min damage", rank_one_magic_tooltip.contains("next rank: +%d%% weapon damage min" % magic_min_delta))
	_assert_true("rank 1 tooltip includes next-rank max damage", rank_one_magic_tooltip.contains("next rank: +%d%% weapon damage max" % magic_max_delta))
...
func _visible_skill_ids_for_class(class_id: String) -> Array:
	var out: Array = []
	for raw_skill_id in SkillRulesLoaderScript.skill_ids_by_tree():
