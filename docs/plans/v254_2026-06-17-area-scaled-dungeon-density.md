...
- [x] Other over-limit file from `.maintainability/file-size-baseline.tsv`: `tools/validate_shared.py` stayed within allowance after extracting `tools/dungeon_density.py`
...
Autoloop batch note: final `make ci` is deferred to the `$autoloop` batch gate after the selected
slice commit.
