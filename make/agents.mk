...
bot-client: ## Run Godot client bot scenarios against a running server (requires db-up + server); pass scenario=<id|all>, HEADLESS=1 for headless/CI
	GODOT="$(GODOT)" BASE_URL="$(BASE_URL)" DEV_TOKEN="$(DEV_TOKEN)" \
	SCENARIO="$(or $(SCENARIO),$(scenario),all)" \
	HEADLESS="$(or $(HEADLESS),0)" ./scripts/bot_client.sh
