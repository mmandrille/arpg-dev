...
func (s *Sim) startPoisonDot(player *entity, target *entity, skillID string, def SkillDef, sourceDamage int, correlationID string, res *TickResult) {
	...
	s.startDotStatus(target, "poisoned", skillID, player.id, rank, damage, 10, duration, correlationID, res)
	s.startRogueMark(player, target, skillID, def, rank, correlationID, res)
	s.replicateDotStatus(player.id, target, "poisoned", skillID, rank, damage, 10, duration, correlationID, res)
}
...
func (s *Sim) resolvePlayerAttackDamageForSlot(slot string) DamageRange {
	...
	statMin, statMax := weaponStatDamageBonus(s.rules, stats, item)
	minDamage := int(math.Floor(statMin + baseMin + minRoll))
	maxDamage := int(math.Floor(statMax + baseMax + maxRoll))
	if minDamage < 0 {
		minDamage = 0
	}
	if maxDamage < minDamage {
