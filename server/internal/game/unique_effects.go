...
func (s *Sim) triggerUniqueEffectsAfterHeroDamage(target *entity, playerID uint64, skillID string, corr string, res *TickResult, outcome combatResolution, source uniqueHeroDamageSource) {
	...
	for _, effectID := range s.equippedUniqueEffectIDs(playerID) {
		...
		default:
			def, ok := s.liveUniqueEffect(effectID, "on_skill_hit_status")
			if ok {
				s.applySkillHitStatusUnique(target, playerID, skillID, def, outcome.Damage, corr, res)
			}
		}
	}
}
