...
func (s *Sim) handleSwapWeaponSet(in Input, res *TickResult) {
	...
	res.Changes = append(res.Changes,
		Change{Op: OpEquippedUpdate, Slot: mainHandSlot, ItemInstanceID: stringPtrFromUint(s.equipped[mainHandSlot])},
		Change{Op: OpEquippedUpdate, Slot: offHandSlot, ItemInstanceID: stringPtrFromUint(s.equipped[offHandSlot])},
