// Package ids generates prefixed ULIDs for platform identities (accounts,
// characters, sessions, messages, correlation ids) and opaque access tokens.
//
// These identities are NOT part of the deterministic authoritative simulation
// (which uses a per-session monotonic counter for entity ids); they identify
// persistence and protocol objects, so drawing from crypto/rand entropy is
// correct here.
package ids
...
// New returns a prefixed ULID, e.g. New("sess") -> "sess_01J....".
...
// Token returns an opaque, URL-safe random access token (256 bits, hex).
func Token() string {
	var b [32]byte
	_, _ = rand.Read(b[:])
...
// encode renders the 128-bit value as the canonical 26-character ULID string.
