...
func TestAccountResourceWalletAddSpendAndScope(t *testing.T) {
	...
	added, err := s.AddAccountResource(ctx, acct.ID, "skill_badge", 2)
	if err != nil {
	...
	added, err = s.AddAccountResource(ctx, acct.ID, "skill_badge", 1)
	if err != nil {
	...
	if _, err := s.AddAccountResource(ctx, acct.ID, "skill_badge", 0); !errors.Is(err, store.ErrConflict) {
		t.Fatalf("add zero resource = %v, want ErrConflict", err)
	}

	spent, err := s.SpendAccountResource(ctx, acct.ID, "skill_badge", 2)
	if err != nil {
	...
	if _, err := s.SpendAccountResource(ctx, acct.ID, "skill_badge", 2); !errors.Is(err, store.ErrConflict) {
		t.Fatalf("overspend resource = %v, want ErrConflict", err)
	}
	if _, err := s.SpendAccountResource(ctx, other.ID, "skill_badge", 1); !errors.Is(err, store.ErrConflict) {
	...
	if len(resources) != 1 || resources[0].ResourceID != "skill_badge" || resources[0].Amount != 1 {
		t.Fatalf("resources = %+v, want one skill_badge amount 1", resources)
	}
