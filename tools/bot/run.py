...
def _parse_server_perf_log(log_path: Path) -> tuple[int, list[float], list[float]]:
    overrun_count = 0
...
def assert_soak_server_perf(scenario_id: str, soak_config: dict[str, Any]) -> None:
    ...
    overrun_count, persist_samples, overrun_samples = _parse_server_perf_log(path)
...
async def run_crowded_skill_overlap_lab(
    *,
    ...
    try:
        ...
        overlap_rounds = 3
        volley_casts = 0
        for _round in range(overlap_rounds):
        ...
        assert_soak_server_perf(scenario.id, soak_config)
    finally:
        for peer in peers:
            try:
                await close_coop_peer(peer)
