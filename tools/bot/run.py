...
async def execute_step(
    ...
    *,
    ...
    if action == "set_companion_stance":
        from tools.bot.companion_commands import set_companion_stance
    ...
    if action == "channel_skill_path":
        ...
        await execute_channel_skill_path(ws, session_id, state, step, loop, make_envelope, wait_for_accept, wait_for_matching_event, pump_one)
        return
...
def _combat_soak_helpers() -> dict[str, Any]:
    return {
        "log": log,
        "seed_debug_progression": seed_debug_progression,
        "create_coop_session": create_coop_session,
        "join_coop_session": join_coop_session,
        "connect_coop_peer": connect_coop_peer,
        "wait_coop_until": wait_coop_until,
        "send_coop_intent": send_coop_intent,
        "pump_coop": pump_coop,
        "close_coop_peer": close_coop_peer,
...
async def run_crowded_skill_overlap_lab(
    *,
    ...
    from tools.bot.combat_soak_runtime import run_crowded_skill_overlap_lab as run_crowded_skill_overlap_lab_impl

    return await run_crowded_skill_overlap_lab_impl(
        client=client,
        base_url=base_url,
        tokens=tokens,
...
async def run_six_player_boss_combat_soak(
    *,
    ...
    from tools.bot.combat_soak_runtime import run_six_player_boss_combat_soak as run_six_player_boss_combat_soak_impl

    return await run_six_player_boss_combat_soak_impl(
        client=client,
        base_url=base_url,
        tokens=tokens,
        debug_token=debug_token,
        ...
        for scenario in selected:
            ...
            try:
                if scenario.id == "skill_visual":
                    ...
                    token = tokens[0]
                elif scenario.id == "crowded_skill_overlap_lab":
                    tokens = []
                    character_ids = []
                    peer_count = max(2, scenario.peer_count)
                    replay_email = scenario_email(args.email, f"{scenario.id}-peer-0")
                    for index in range(peer_count):
                        ...
                        character_ids.append(
                            ensure_character(client, peer_token, f"Overlap Peer {index + 1}", scenario.character_class)
                        )
                    sess, observed = asyncio.run(run_crowded_skill_overlap_lab(
                        client=client, base_url=args.base_url, tokens=tokens,
                        debug_token=args.debug_token, scenario=scenario, character_ids=character_ids,
                    ))
                    token = tokens[0]
                elif scenario.id == "six_player_boss_combat_soak":
                    tokens = []
                    character_ids = []
                    ...
                    for index in range(peer_count):
                        ...
                        _, peer_token = dev_login(client, email, args.dev_token)
                        tokens.append(peer_token)
                        loadout = loadouts[index] if index < len(loadouts) else {}
                        class_id = str(loadout.get("character_class", scenario.character_class or "barbarian"))
                        character_ids.append(ensure_character(client, peer_token, f"Soak Peer {index + 1}", class_id))
                    sess, observed = asyncio.run(run_six_player_boss_combat_soak(
                        client=client, base_url=args.base_url, tokens=tokens,
                        debug_token=args.debug_token, scenario=scenario, character_ids=character_ids,
                    ))
                    token = tokens[0]
