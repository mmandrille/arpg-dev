        ...
        for unique_id, unique in unique_defs.items():
            ...
            if not isinstance(fixed_effect_ids, list):
                ...
                for effect_id in fixed_effect_ids:
                    effect = unique_effect_defs.get(effect_id)
                    if effect is None:
                    ...
                    if item_type not in effect.get("compatible_item_types", []):
                        report.fail("unique_items fixed_effect_ids", f"{unique_id}: effect {effect_id} incompatible with {template_id} type {item_type}")
                        failed_uniques = True
